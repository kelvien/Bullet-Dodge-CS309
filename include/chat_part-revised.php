...
<p class="text-center" id="expand_friend_button">Friends <?php echo "(".$userFriends->num_rows.")"; ?></p>
<?php
...
$(".chat_friend").on("click", function(){
	...
	if($(".chat_with_friend_title").size() > 0){
		$(".chat_with_friend_title").each(function(i, e){
			if($(e).children().first().clone().children().remove().end().text().trim() == name){
				found = true;
			}
		});
		if(found == false && stat == "● ONLINE"){
